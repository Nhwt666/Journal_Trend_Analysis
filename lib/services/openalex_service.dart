...
class OpenAlexService {
  ...
  Future<SearchResult> searchWorks(
    ...
    final params = <String, String>{
      ...
      'sort': sort.apiValue,
      // Tell OpenAlex to only return the fields the list view and the
      // dashboard actually consume. Each dropped field is also dropped
      // from local parsing, so the smaller payload means less network,
      // less JSON decoding and less GC pressure when scrolling.
      //
      // We deliberately keep `topics` and `authorships` so the
      // dashboard can show a per-field breakdown and per-author top
      // lists. We still drop:
      //   - abstract_inverted_index  (large; only needed on detail)
      //   - language                 (unused)
    ...
    if (filters.fromYear != null) {
      filterParts.add('from_publication_year:${filters.fromYear}');
    }
    if (filters.toYear != null) {
      filterParts.add('to_publication_year:${filters.toYear}');
    }
    ...
    return SearchResult(
      publications: results
          .map((e) =>
              Publication.fromSummaryJson(e as Map<String, dynamic>))
          .toList(),
      totalCount: total,
      hasMore: hasMore,
    );
  ...
  Future<List<Topic>> fetchFeaturedTopics({int perPage = 50}) async {
    ...
    final uri =
        Uri.parse('$_baseUrl/topics').replace(queryParameters: params);
