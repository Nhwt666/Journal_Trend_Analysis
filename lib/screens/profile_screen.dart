...
/// Minimal Settings/About screen. Exposes the same dark-mode toggle
/// the AppBar already offers (so users can change it from here too)
/// and a couple of at-a-glance counters so the screen earns its keep.
  ...
  Future<void> _confirmReset(
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Reset activity stats?'),
        content: const Text(
          'This clears the searches-run counter and re-anchors '
          'the "first opened" date. Bookmarks, history and '
          'recents are not touched.',
        ),
        actions: [
          TextButton(
    ...
    if (ok == true) {
      await profile.resetStats();
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Activity stats reset.')),
        );
      }
    }
  }
